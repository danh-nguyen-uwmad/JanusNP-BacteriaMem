...
% cut the half part
% del1_index=find(XYZ(:,3)> 0); % 50-50 Janus
% del1_index=find(XYZ(:,3)> 0); % 50-50 Janus
% del1_index=find(XYZ(:,3) > -r_p*0.81); % 80-20 (20 hydrophopic)
% del1_index=find(XYZ(:,3) > -r_p*0.95); % 90-10 (10 hydrophopic)
% del1_index=find(XYZ(:,3) > -r_p*0.5); % 67-33 (33 hydrophopic)
% XYZ(del1_index,:)= [];
% 
n_surf=size(XYZ,1);
number=1:n_surf;
...
xyz_p=xyz_p*rescale;
NP_new=[];
N1=0;             % total # of NP_inner core atoms
...
% Displace Au-surface id and location here???
% check the bonding of Au-Au inner core
...
% genetate the positive half sphere
% n_density2 =  0.025;  %density of positive charged bead
% n_p2=round(4*pi*r_p^2*n_density2);
% index2        = [linspace(0,n_p2-1,n_p2)]';

z_philic = z(index_philic);
golden_angle2 = pi*(3-sqrt(5));
theta2        = golden_angle2*index_philic;
%z2            = [linspace(1-1/n_p2,1/n_p2-1,n_p2)]';
...
% cut the half part
%del2_index         = find(XYZ2(:,3)<0); %50-50
%del2_index         = find(XYZ2(:,3)<-r_p*0.81); %80-20 (80 hydrophilic)
%del2_index         = find(XYZ2(:,3)<-r_p*0.95); %90-10 (90 hydrophilic)
%del2_index         = find(XYZ2(:,3)<-r_p*0.5); %67-33 (33 hydrophilic)
...
% AuNP_atom(:,2)=1;   % mol
% AuNP_atom(:,3)=1;   % type
% AuNP_atom(:,4)=0.0;   % charge
% AuNP_atom(:,5:7)= all_data(:,5:7); % position
% AuNP_CG_type = [];

% for i = 1 : N
% temp = string(AuNP_CG_name{i});
% AuNP_CG_type = vertcat(AuNP_CG_type, temp);
% end 
...
% Connecting points with a distance of 0.445 from the PMB_pos points
%r_p = 54; % Angstrong distance from COM to the PMB (54 Ang = 5.4 nm)
%n_density =  0.001; %0.005;  %density of PMBs on the surface
%n_p = round(4 * pi * r_p^2 * n_density);
%index        = [linspace(0, n_p - 1, n_p)]';
% golden_angle = pi * (3-sqrt(5));
...
% cut the half part
%del_index        = find(XYZ3(:,3)<5); % to remove near to the interface with alkyl chain
%XYZ3(del_index,:) = [];
n_p             = size(XYZ3,1);
...
% 1. Read PMB xyz 1.2 2. Rotate the PMB x, y, z by the angle between vector
% of (COM-point) and PMB vector 3. Translocate the C1 of PMB to the
% connecting point from C1 atom (PMB)to that connecting point
...
%     
% %fprintf(fid,'%5d%-5s%5s%5d%8.3f%8.3f%8.3f\n',1,'MOL',strcat('P4'),all_data(i,1),
% %all_data(i,5:7));
% fprintf(fid,'%5d%-5s%5s%5d%8.3f%8.3f%8.3f\n',1,'MOL', AuNP_CG_type(i), AuNP_atom(i,1), AuNP_atom(i,5:7));
%     
% end
...
for i=1:size(all_data,1)
    ...
    % fprintf(fid,'%5d%5s%5s%5d%8.3f%8.3f%8.3f\n',1,'MOL',strcat('B',num2str(all_data(i,3))),all_data(i,1), all_data(i,5:7)/10);
    ...
    elseif all_data(i,3)==4
        s4=s4+1;
        %fprintf(fid,'%5d%-5s%5s%5d%8.3f%8.3f%8.3f\n',1,'MOL',strcat('Qd'),all_data(i,1), all_data(i,5:7)/10);
        fprintf(fid,'%5d%-5s%5s%5d%8.3f%8.3f%8.3f\n',1,'MOL',strcat('N0'),all_data(i,1), all_data(i,5:7)/10);
...
Au_second = [];
Au_Au = [];
%a = norm(NP_inner_core(1,3:5)-NP_inner_core(1,3:5));

for i = (n_surf+1) : (n_surf + Au_inner_total-1)
    for j = i+1 : (n_surf + Au_inner_total)
        %if norm(all_data(j,5:7)-all_data(i,5:7)) < 5.8
...
fprintf(fid,'; molname  	nrexcl\n');             % !!!!!__PLEASE ADJUST THE nrexcl (to control FENE-like LJ potential)__!!!!!
fprintf(fid,'MOL   1\n');
fprintf(fid,'\n');
